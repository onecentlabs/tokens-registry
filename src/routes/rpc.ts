...
export class RPCFetch {
  ...
  constructor(metadataPath?: string) {
    ...
    this.rpcMap = {}
    const defaultPath = './data/chains/metadata.json'
    const pathToUse = metadataPath && existsSync(metadataPath) ? metadataPath : (existsSync(defaultPath) ? defaultPath : './chains/metadata.json')
    this.loadRPCs(pathToUse)
  }
  ...
  private loadRPCs(metadataPath: string) {
    try {
      ...
      chains.forEach((chain: any) => {
        ...
        } else if (chain.properties && chain.properties.rpcs) {
          this.rpcMap[chain.id] = chain.properties.rpcs
        }
      })
